        ...
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="relative cursor-pointer">
              ...
              <span className="absolute -top-2 -right-2 bg-[#ff004c] text-[8px] font-black w-4 h-4 rounded-full flex items-center justify-center">0</span>
            </div>
          </div>
        </div>
      </nav>
      ...
      <section className="pt-32 pb-24">
        <div className="container mx-auto px-6">
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
            ...
            <div className="w-full lg:w-1/2">
              <div className="sticky top-32">
                <div className="relative group">
                  <div className="aspect-[4/5] bg-gradient-to-br from-red-950/40 to-black rounded-[40px] border border-white/10 relative overflow-hidden shadow-2xl flex items-center justify-center">
                    ...
                    <div className="relative z-10 text-center px-8">
                       <h2 className="font-display text-4xl font-black italic uppercase tracking-tighter mb-2 transform group-hover:-translate-y-2 transition-transform">{selectedProduct.name}</h2>
                       <p className="text-[#ff004c] font-black uppercase tracking-[0.3em] text-[10px]">{selectedProduct.claim}</p>
                    </div>

                    {/* 3D Can Visual Component */}
                    <div className="absolute -bottom-20 right-10 w-32 h-[300px] bg-gradient-to-b from-[#ff004c] to-[#3b0000] rounded-[24px] shadow-2xl border-2 border-white/10 transform rotate-12 group-hover:rotate-6 transition-transform duration-500 hidden sm:block">
                      <div className="absolute top-4 w-full text-center">
                        <span className="font-display text-xl font-black text-white/80">A</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 grid grid-cols-4 gap-4">
                  {displayProducts.map((p) => (
                    <button 
                      ...
                      className={`aspect-square rounded-2xl border overflow-hidden transition-all ${selectedProductId === p.id ? 'border-[#ff004c] ring-2 ring-[#ff004c]/20' : 'border-white/10 opacity-50 grayscale hover:opacity-100 hover:grayscale-0'}`}
                    >
            ...
            <div className="w-full lg:w-1/2">
              <div className="mb-8">
                <div className="inline-block px-3 py-1 rounded-full border border-red-900/50 bg-red-950/20 text-[#ff004c] text-[10px] font-black uppercase tracking-widest mb-6 italic">
                  Premium Selection
                </div>
                <h1 className="font-display text-4xl lg:text-5xl font-black uppercase tracking-tighter italic mb-4">
                  {selectedProduct.name}
                </h1>
                <p className="text-[#ff004c] font-bold text-sm uppercase tracking-widest mb-6 italic">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {displayProducts.map((p) => (
                    <div 
                      key={p.id}
                      onClick={() => setSelectedProductId(p.id)}
                      className={`p-5 rounded-2xl border transition-all cursor-pointer flex justify-between items-center group ${selectedProductId === p.id ? 'border-[#ff004c] bg-red-950/10' : 'border-white/5 bg-white/[0.02] hover:border-white/20'}`}
                    >
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-4 border ${selectedProductId === p.id ? 'bg-[#ff004c] border-[#ff004c]' : 'border-gray-700'}`}></div>
                        <div>
                           <h4 className="font-bold text-sm uppercase italic tracking-tight group-hover:text-white transition-colors">{p.name}</h4>
              ...
              <div className="space-y-4">
                <button 
                  ...
                  disabled={isAdding}
                  className={`w-full py-6 rounded-full font-black uppercase tracking-widest text-lg transition-all shadow-[0_10px_40px_rgba(255,0,76,0.2)] flex items-center justify-center ${isAdding ? 'bg-green-600 text-white' : 'bg-white text-black hover:bg-[#ff004c] hover:text-white active:scale-95'}`}
                >
                  {isAdding ? (
